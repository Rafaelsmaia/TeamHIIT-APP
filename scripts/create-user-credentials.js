 ...
 * 
 * Uso: node scripts/create-user-credentials.js rosana-lobato@hotmail.com
 */
...
try {
  ...
  // Tentar carregar de functions/serviceAccountKey.json
  try {
    const altPath = path.join(__dirname, '..', 'functions', 'serviceAccountKey.json');
    serviceAccount = require(altPath);
    console.log('✅ serviceAccountKey.json encontrado em functions/');
  } catch (altError) {
    console.error('❌ Erro ao carregar serviceAccountKey.json:', error.message);
    console.error('💡 Certifique-se de que o arquivo serviceAccountKey.json existe em ./ ou ./functions/');
    process.exit(1);
  }
}
...
async function createUserCredentials(email) {
  try {
    ...
    try {
      ...
      // Gerar nova senha
      tempPassword = generateRandomPassword(name);
    ...
    await db.collection('user_credentials').doc(firebaseUser.uid).set(credentialsData);
    await db.collection('user_credentials').doc(email).set(credentialsData);
    ...
    // 6. Atualizar status no Firestore
    await db.collection('user_credentials').doc(firebaseUser.uid).update({
      sent: true,
      sentAt: admin.firestore.FieldValue.serverTimestamp()
    });
    await db.collection('user_credentials').doc(email).update({
      sent: true,
      sentAt: admin.firestore.FieldValue.serverTimestamp()
    });
...
if (!emailArg) {
  console.error('❌ Uso: node scripts/create-user-credentials.js email@exemplo.com');
  process.exit(1);
}

createUserCredentials(emailArg).then(() => {
  process.exit(0);
}).catch(error => {
